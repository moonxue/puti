...
namespace airlib_rpclib
{
    ...
    public:
        ...
        struct RotateTurbineState {
            std::string name;
            double angle;
            bool  rotating;
            Pose blade_pivot_pose;

            MSGPACK_DEFINE_MAP(name, angle, rotating, blade_pivot_pose);

            RotateTurbineState(){

            };

            RotateTurbineState(const msr::airlib::RotateTurbineState s){
                name =s.name;
                angle=s.angle;
                rotating =s.rotating;
                blade_pivot_pose =s.blade_pivot_pose;
            }

            msr::airlib::RotateTurbineState to(){
                msr::airlib::RotateTurbineState s;
                s.name =name;
                s.angle =angle;
                s.rotating =rotating;
                s.blade_pivot_pose =blade_pivot_pose.to();

...
namespace airlib
{
        ...
        : RpcLibServerBase(api_provider, server_address, port)
    {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("moveOnPathWithGimbalPose", [&](const vector<MultirotorRpcLibAdaptors::Vector3r>& path,const vector<MultirotorRpcLibAdaptors::Pose>& drone_pose, float velocity, const vector<MultirotorRpcLibAdaptors::Pose>& camera_poses, float timeout_sec, DrivetrainType drivetrain, const MultirotorRpcLibAdaptors::YawMode& yaw_mode, float lookahead, 
                                                        float adaptive_lookahead, const std::string& vehicle_name, const std::string& camera_name) -> bool {
            vector<Vector3r> conv_path;
            vector<Pose> conv_drone_poses,conv_camera_poses;
            MultirotorRpcLibAdaptors::to(path, conv_path);
            MultirotorRpcLibAdaptors::to(drone_pose,conv_drone_poses);
            MultirotorRpcLibAdaptors::to(camera_poses,conv_camera_poses);
            auto world_sim_api=getWorldSimApi();
            return getVehicleApi(vehicle_name)->moveOnPathWithGimbalPose(conv_path,conv_drone_poses, velocity,conv_camera_poses, timeout_sec, drivetrain, yaw_mode.to(), lookahead, adaptive_lookahead,world_sim_api,vehicle_name,camera_name);
        });

...
namespace airlib
{
                                       ...
                                       float lookahead, float adaptive_lookahead)
    {
        ...
        */
        ...
        bool MultirotorApiBase::moveOnPathWithGimbalPose(const vector<Vector3r>& path, const vector<Pose>& drone_poses,float velocity, vector<Pose>& camera_poses, float timeout_sec, DrivetrainType drivetrain, const YawMode& yaw_mode,
                                       float lookahead, float adaptive_lookahead,  msr::airlib::WorldSimApiBase* world_sim_api,const std::string& vehicle_name, const std::string& camera_name)
    {
        SingleTaskCall lock(this);

        //validate path size
        if (path.size() == 0) {
            Utils::log("moveOnPathWithGimbalPose terminated because path has no points", Utils::kLogLevelWarn);
            return true;
        }

        if (drone_poses.size()==0 )
        {
            Utils::log("moveOnPathWithGimbalPose terminated because drone poses are empty.",Utils::kLogLevelWarn);
            return true;
        }
        if (camera_poses.size()==0)
        {
            Utils::log("moveOnPathWithGimbalPose terminated because camera poses are empty.",Utils::kLogLevelWarn);
            return true;
        }
        

        if ((drone_poses.size()!=path.size()) ||(camera_poses.size()!=path.size()))
        {
            Utils::log("moveOnPathWithGimbalPose terminated because sizes of poses and points of path are not equal.",Utils::kLogLevelWarn);
            return true;
        }
        ...
        // add current pose as starting pose.
        //query CameraInfo
        CameraDetails control_camera(camera_name,vehicle_name,false);
        vector<Pose> camera_poses_3d;
        Pose initialCameraPose;
        initialCameraPose.position = Eigen::Vector3f(0.5,0,0.3);
        initialCameraPose.orientation = Eigen::Quaternion<float,2>(1,0,0,0);
        camera_poses_3d.push_back(initialCameraPose);
        ...
        for (uint i = 0; i < path.size(); ++i) {
            point = path.at(i);
            EulerAngles angles= toEulerAngles(drone_poses.at(i).orientation);
            PathSegment path_seg(path3d.at(i), point, velocity, path_length);
            path_length += path_seg.seg_length;
            path_segs.push_back(path_seg);
            path3d.push_back(point);
        ...
        for (uint i = 0; i < camera_poses.size(); ++i)
        {
            camera_poses_3d.push_back(camera_poses.at(i));
        }
        ...
        /**
             * set drone pose here, 
             *Commanded pose is expected to contain position in NED, relative to default camera location.
             @ref https://github.com/microsoft/AirSim/pull/2710
             @author Cheny
            */
        //until we are at the end of the path (last seg is always zero size)
        while (!waiter.isTimeout() && (next_path_loc.seg_index < path_segs.size() - 1 || goal_dist > 0)) { //current position is approximately at the last end point
            ...
            Pose cam_pose;
            cam_pose.position=Eigen::Vector3f(5.0,0.0,0.3);
            cam_pose.orientation = camera_poses_3d.at(cur_path_loc.seg_index).orientation; // alter orientation and omit position only.
            world_sim_api->setCameraPose(cam_pose,control_camera);
           // send drone command to get to next lookahead.
            moveToPathPosition(next_path_loc.position, seg_velocity, drivetrain, yaw_mode, path_segs.at(cur_path_loc.seg_index).start_z);
            EulerAngles angles = toEulerAngles(drone_poses.at(next_path_loc.seg_index).orientation);
            moveByRollPitchYawThrottle(angles.roll,angles.pitch,angles.yaw,0.5,1);
            //sleep for rest of the cycle
            if (!waiter.sleep())
                return false;

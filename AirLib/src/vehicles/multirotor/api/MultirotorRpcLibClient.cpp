    ...
    namespace msr
{
    namespace airlib
    {
        ...
        MultirotorRpcLibClient* MultirotorRpcLibClient::moveOnPathWithGimbalPoseAsync(const vector<Vector3r>& path,const vector<Pose>& drone_poses, float velocity, const vector<Pose>& camera_poses,float timeout_sec,
                                                DrivetrainType drivetrain, const YawMode& yaw_mode,
                                                float lookahead, float adaptive_lookahead, const std::string& vehicle_name,const std::string& camera_name)
        {
            vector<MultirotorRpcLibAdaptors::Vector3r> conv_path;
            vector<MultirotorRpcLibAdaptors::Pose>conv_drone_poses, conv_camera_poses;
            MultirotorRpcLibAdaptors::from(path, conv_path);
            MultirotorRpcLibAdaptors::from(drone_poses,conv_drone_poses);
            MultirotorRpcLibAdaptors::from(camera_poses,conv_camera_poses);
            pimpl_->last_future = static_cast<rpc::client*>(getClient())->async_call("moveOnPathWithGimbalPose",conv_path,conv_drone_poses,velocity,conv_camera_poses,timeout_sec,drivetrain,MultirotorRpcLibAdaptors::YawMode(yaw_mode),lookahead,adaptive_lookahead,vehicle_name,camera_name);
            return this;
        }

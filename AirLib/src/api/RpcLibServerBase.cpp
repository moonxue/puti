...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
        ...
        pimpl_->server.bind("simGetRotateTurbineAngle",[&](const std::string& object_name)-> double {
            return getWorldSimApi()->getRotateTurbineAngle(object_name);
        });

        pimpl_->server.bind("simSetEnableTurbineRotate",[&](const std::string& object_name,const bool& en)-> void {
            return getWorldSimApi()->setEnableTurbineRotate(object_name,en);
        });

        pimpl_->server.bind("simListRotateTurbines",[&]()-> std::vector<std::string> {
            return getWorldSimApi()->listRotateTurbines();
        });

        pimpl_->server.bind("simGetRotateTurbineState",[&](const std::string& object_name)->RpcLibAdaptorsBase::RotateTurbineState {
            auto turbine_state = getWorldSimApi()->getRotateTurbineState(object_name);
            return RpcLibAdaptorsBase::RotateTurbineState(turbine_state);
        });

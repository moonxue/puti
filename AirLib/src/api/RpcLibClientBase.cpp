    ...
    namespace msr
{
    namespace airlib
    {
        ...
        double RpcLibClientBase::simGetRotateTurbineAngle(const std::string& object_name){
            return pimpl_->client.call("simGetRotateTurbineAngle", object_name).as<double>();
        }

        void RpcLibClientBase::simSetEnableTurbineRotate(const std::string object_name, const bool en){
            pimpl_->client.call("simSetEnableTurbineRotate", object_name,en);
        }
        std::vector<std::string> RpcLibClientBase::simListRotateTurbines(){
            return pimpl_->client.call("simListRotateTurbines").as<vector<string>>();
        }

        msr::airlib::RotateTurbineState RpcLibClientBase::simGetRotateTurbineState(const std::string& object_name){
            auto turbine_state = pimpl_->client.call("simGetRotateTurbineState",object_name).as<RpcLibAdaptorsBase::RotateTurbineState>();
            return turbine_state.to();
        }

...
void ASimModeBase::loseWindowFocus(){
   APlayerController* playerController = GetWorld()->GetFirstPlayerController();
    if (playerController)
    {
        ...
        // allow cursor move outside window.
...
void ASimModeBase::setupInputBindings()
{
    ...
    UAirBlueprintLib::BindActionToKey("InputEventToggleWindowFocus",EKeys::Escape,this, &ASimModeBase::loseWindowFocus);
}
...
APawn* ASimModeBase::createVehiclePawn(const AirSimSettings::VehicleSetting& vehicle_setting)
{
    ...
    try
    {
        auto vehicle_bp_class = UAirBlueprintLib::LoadClass(
            ...
            spawned_pawn = static_cast<APawn*>(this->GetWorld()->SpawnActor(
            vehicle_bp_class, &spawn_position, &spawn_rotation, pawn_spawn_params));
            spawned_actors_.Add(spawned_pawn);
    }
    catch(const std::exception& e)
    {
        std::cerr <<"failed to spawn vehicle pawn1!! "<< e.what() << '\n';
    }
...
void ASimModeBase::getRotateTurbineActors(TArray<AActor*>& turbines) const
{
    //derived class should override this method to retrieve types of turbines they support
}

void ASimModeBase::enableTurbineRotate(bool en) const
{
    //derived class should override this method to retrieve types of turbines they support
}

ASimModeBase::RotateTurbineState ASimModeBase::getRotateTurbineState(const std::string& turbine_name) const{
    //derived class should override this method to retrieve types of turbines they support
    return ASimModeBase::RotateTurbineState();
}

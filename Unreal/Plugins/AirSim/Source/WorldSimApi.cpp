...
//begin TurbineApi
double WorldSimApi::getRotateTurbineAngle(const std::string turbine_name){
    TArray<AActor*> turbines;
    double angle=std::numeric_limits<double>::signaling_NaN();
    UAirBlueprintLib::RunCommandOnGameThread([this,&turbines](){
        simmode_->getRotateTurbineActors(turbines);
    },true);
    for(AActor* actor:turbines){
        if(actor->GetName()==FString(turbine_name.c_str())){
            ARotateTurbineBase* turbine = Cast<ARotateTurbineBase>(actor);
            if(turbine){
                angle =turbine->CurrentAngle;
            }else{
                UE_LOG(LogTemp,Warning,TEXT("Couldn't find ARotateTurbineBase object with given name!"));
            }
        }
    }
    return angle;
...
void WorldSimApi::setEnableTurbineRotate(const std::string turbine_name,const bool en){
    TArray<AActor*> turbines;
    double angle=std::numeric_limits<double>::signaling_NaN();
    UAirBlueprintLib::RunCommandOnGameThread([this,&turbines](){
        simmode_->getRotateTurbineActors(turbines);
    ...
    for(AActor* actor:turbines){
        if(actor->GetName()==FString(turbine_name.c_str())||turbine_name.find("All")!=turbine_name.npos){
            ARotateTurbineBase* turbine = Cast<ARotateTurbineBase>(actor);
            if(turbine){
                if(en){
                    turbine->SetEnableRotate();
                }else{
                    turbine->SetDisableRotate();
                }
...
std::vector<std::string> WorldSimApi::listRotateTurbines(){
    TArray<AActor*> turbines;
    std::vector<std::string> turbine_names;
    double angle=std::numeric_limits<double>::signaling_NaN();
    UAirBlueprintLib::RunCommandOnGameThread([this,&turbines](){
        simmode_->getRotateTurbineActors(turbines);
    },true);
    ...
    for(AActor* actor:turbines){
        ...
        turbine_names.push_back(std::string(TCHAR_TO_UTF8(*actor->GetName())));
    }
    return turbine_names;
}

msr::airlib::RotateTurbineState WorldSimApi::getRotateTurbineState(const std::string turbine_name){
    TArray<AActor*> turbines;
    msr::airlib::RotateTurbineState state;
    UAirBlueprintLib::RunCommandOnGameThread([this,&turbines](){
        simmode_->getRotateTurbineActors(turbines);
    },true);
    for(AActor* actor:turbines){
        if(actor->GetName()==FString(turbine_name.c_str())){
            ARotateTurbineBase* turbine = Cast<ARotateTurbineBase>(actor);
            if(turbine){
                state= turbine->getState();
                auto blade_pivot_position=turbine->getBladePivotLocation();
                auto blade_pivot_rotation=turbine->getBladePivotRotation();
                auto ned_pose = simmode_->getGlobalNedTransform().toGlobalNed(FTransform(blade_pivot_rotation,blade_pivot_position));
                state.blade_pivot_pose.position=ned_pose.position;
                state.blade_pivot_pose.orientation=ned_pose.orientation;
            }else{
                UE_LOG(LogTemp,Warning,TEXT("Couldn't find ARotateTurbineBase object with given name!"));
            }
        }
